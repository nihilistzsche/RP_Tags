...
Module:WaitUntil("after UTILS_GET", function(self, event, ...)
	...
	local function isParty(line)
		return line.e == "PARTY"
			or line.e == "PARTY_LEADER"
			or line.e == "INSTANCE_CHAT"
			or line.e == "INSTANCE_CHAT_LEADER"
			or line.e == "RAID"
	...
	local function isSay(line)
		return line.e == "SAY" or isEmotedSay(line)
	end
	...
	local function isTextEmoteToMe(line)
		return line.e == "TEXT_EMOTE"
			and line.m:find("you")
			and not (
				line.m:find("orders you to open fire.")
				or line.m:find("asks you to wait.")
				or line.m:find("tells you to attack")
				or line.m:find("motions for you to follow.")
				or line.m:find("looks at you with crossed eyes")
			)
	end

	local function msg(line, u, quotedOnly)
		return (line.e == "EMOTE" and (name(u) .. " ") or "") .. line.m:match(quotedOnly and '"(.-)"' or "(.+)")
	end

	local function listenerCount(u)
		local count = 0
		if Main.chat_history[UnitName(u)] then
			for _, line in ipairs(Main.chat_history[UnitName(u)]) do
				count = count + (isAnyOfTheAbove(line) and 1 or 0)
			end
		end
		return count
	end
	...
	local function listenerLast(u)
		if listenerCount(u) > 0 then
			local history = Main.chat_history[UnitName(u)]
			for i = #history, 1, -1 do
				if isAnyOfTheAbove(history[i]) then
					return msg(history[i], u)
				end
			end
		end
		return ""
	...
	local function listenerWhisper(u)
		if listenerCount(u) > 0 then
			local history = Main.chat_history[UnitName(u)]
			for i = #history, 1, -1 do
				if isWhisper(history[i]) then
					return msg(history[i])
				end
			end
		end
		return ""
	...
	local function listenerSay(u)
		if listenerCount(u) > 0 then
			local history = Main.chat_history[UnitName(u)]
			for i = #history, 1, -1 do
				if isSay(history[i]) then
					return msg(history[i], nil, isEmotedSay(history[i]))
				end
			end
		end
		return ""
	...
	local function listenerToYou(u)
		if listenerCount(u) > 0 then
			...
			for i = #history, 1, -1 do
				if
					isSay(history[i]) and isToMe(history[i])
				...
				then
					return msg(history[i], u, isEmotedSay(history[i]))
				end
			end
		end
		return ""
	...
	local function listenerColor(u)
		local count = listenerCount(u)
		if count > 0 then
			local last = Main.chat_history[UnitName(u)][count]
...
Module:WaitUntil("DATA_CONST", function(self, event, ...)
	RPTAGS.CONST.CONFIG.LISTENCOLOR_STEP = 5
end)

Module:WaitUntil("before OPTIONS_COLORS", function(self, event, ...)
	local loc = RPTAGS.utils.locale.loc
	local Config = RPTAGS.utils.config

	RPTAGS.utils.modules.addOptions(
		addOnName,
		"colors",
		{
			listenerColors = {
				...
				args = {
					...
					stepSize = {
						...
						step = 1,
						get = function()
							return Config.get("LISTENCOLOR_STEP")
						end,
						set = function(info, value)
							Config.set("LISTENCOLOR_STEP", value)
						end,
					},
				},
			},
...
Module:WaitUntil("DATA_TAGS", function(self, event, ...)
