...
local Target = C_AddOns.GetAddOnMetadata(addOnName, "X-RPQTarget")

Module:WaitUntil("MODULE_G", function(self, event, ...)
	...
	addOptions(
		addOnName,
		"general.keybind",
		{ ic_ooc = build_keybind("ic_ooc"), mouseover_open = build_keybind("mouseover_open") }
	)
end)

Module:WaitUntil("CORE_KEYBIND", function(self, event, ...)
	...
	registerKeybind("IC_OOC", function()
		linkHandler("addon://totalRP3?ic_ooc")
		notify(
			"You are now " .. RPTAGS.utils.get.color.status("player") .. RPTAGS.utils.get.text.status("player") .. "|r."
		)
	end)

	registerKeybind("MOUSEOVER_OPEN", function()
