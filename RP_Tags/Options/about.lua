...
RPTAGS.queue:WaitUntil("OPTIONS_ABOUT", function(self, event, ...)
	-- RPQ -----------------------------------------------------------------------------------------------------------------------------
	--
	...
	local AboutHeader = Markdown(
		string.format(
			...
			C_AddOns.GetAddOnMetadata(addOnName, "X-VersionDate")
		)
	)
	...
	local function build_addOn(addOn)
		...
		local version = addOn.version
				and addOn.version:gsub(" ?alpha ?(.+)", "|cffff0000a%1|r"):gsub(" ?beta ?(.*)", "|cffffff00b%1|r")
			or ""
		local rpqType = addOn.rpqType and loc("RPQ_TYPE_" .. addOn.rpqType:upper()) or ""
		local desc = addOn.desc or ""

		if rpqType ~= loc("RPQ_TYPE_CORE") and addOn.rpqId then
			name = RPTAGS.CONST.NBSP .. name
		end

		local links = {}
		for i, func in pairs({ "open", "options", "help" }) do
			if hasFunc(addOn, func) then
				table.insert(links, "[" .. func .. "](addon://" .. addOn.name .. "?" .. func .. ")")
			end
		end

		if not addOn.enabled then
			name, version, rpqType, desc = grey(name), grey(version), grey(rpqType), grey(desc)
			links = {}
		end
	...
	local function build_category(cat)
		for addOnName, addOnData in pairs(RPTAGS.cache.addOns[cat]) do
			args[addOnName] = build_addOn(addOnData)
			local target = RPTAGS.cache.addOns.other[addOnData.target]

			if target then
				args[addOnData.target] = build_addOn(target)
			end
		end
	...
	for category, _ in pairs(RPTAGS.queue:GetModuleTypes()) do
		local category_0 = category .. "_0"
		if RPTAGS.cache.addOns[category] then
			build_category(category)
		end
		if RPTAGS.cache.addOns[category_0] then
			build_category(category_0)
		end
	end

	if RPTAGS.cache.addOns.targets ~= {} then
		args.blankBeforeTargets = Blank_Line()
	...
	local Version = { type = "group", name = loc("OPT_VERSION"), order = source_order(), args = args }

	local function get_debug_info()
		local value =
			RPTAGS.cache.debug:gsub("\n+$", ""):gsub("^\n*%s*", ""):gsub("|cff%x%x%x%x%x%x", ""):gsub("|r", "")

		if not value:match("^debug [%a ]+$") then
			return debugMessage
		else
			local details = {}
			local FMT = "%-25s = %s"
			...
			local function N(v)
				return type(v) == "string" and v:gsub("|", "||"):gsub("\n", "\\n")
					or type(v) == "number" and v
					or type(v) == "boolean" and v and "true"
					or type(v) == "boolean" and "false"
					or type(v)
			end
			local function P(k, v, lev)
				return string.rep("  ", lev) .. string.format(FMT, N(k), N(v))
			end
			local function H(str, lev)
				return string.rep("  ", lev) .. string.format(HDR, N(str))
			end
			local function addH(str, lev)
				table.insert(details, H(str, lev))
			end
			local function addP(k, v, lev)
				table.insert(details, P(k, v, lev))
			end
			local function addB()
			...
			local function tagList()
				...
				table.insert(
					...
					string.format(
						...
						C_AddOns.GetAddOnMetadata("RP_Tags", "X-VersionDate")
					)
				)
				...
				for _, tagGroup in pairs(RPTAGS.CONST.TAG_DATA) do
					addB()
					...
					for _, tag in pairs(tagGroup.tags) do
						if tag.title then
							addB()
							...
							table.insert(details, "| `[" .. tag.name .. "]` | " .. tag.desc .. " |")
							if not tag.external and not tagGroup.external then
								add(tag.name)
								if tag.label then
									add(tag.name .. "-label")
								end
								if tag.alias then
									for _, aka in ipairs(tag.alias) do
										add(aka)
										if tag.label then
											add(aka .. "-label")
										end
				...
				table.insert(details, "# " .. loc("UI_ALL_TAGS") .. " (" .. #allTags .. ")")
				addB()
				table.insert(details, "\n - " .. table.concat(allTags, "\n - "))
			end

			local sections = { strsplit(" ", value:gsub("^debug ", "")) }
			for i = 1, math.min(#sections, 2) do
				local sec = sections[i]
				if sec:match("^lib") then
					addH(loc("UI_LIBRARIES"), 0)
					for lib in LibStub:IterateLibraries() do
						local _, minor = LibStub(lib)
						addP(lib, minor, 1)
					end
				elseif sec:match("^addon") then
					addH(loc("UI_ADDONS"), 0)
					walk(RPTAGS.cache.addOns, 1)
				elseif sec:match("^config") then
					addH(loc("UI_CONFIG"), 0)
					walk(db, 1)
				elseif sec:match("^tags") then
					tagList()
				else
					addH("Unknown Parameter", 0)
					addP("debug", sec)
				end
				addB()
			end
	...
	local Debug = {
		...
		args = {
			...
			details = {
				...
				order = source_order(),
				get = function(self)
					return get_debug_info(RPTAGS.cache.debug)
				end,
				set = function(self, value)
					RPTAGS.cache.debug = value
				end,
				multiline = 10,
				desc = debugMessage,
			},
