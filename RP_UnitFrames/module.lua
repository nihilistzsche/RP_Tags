...
Module:WaitUntil("ADDON_INIT", function(self, event, ...)
...
Module:WaitUntil("MODULE_C", function(self, event, ...)
	...
	registerFunction(addOnName, "on", function()
		RPTAGS.utils.config.set("DISABLE_RPUF", false)
	end)
	registerFunction(addOnName, "off", function()
		RPTAGS.utils.config.set("DISABLE_RPUF", true)
	end)
	registerFunction(addOnName, "on_off", function()
		RPTAGS.utils.config.set("DISABLE_RPUF", not RPTAGS.utils.config.get("DISABLE_RPUF"))
	end)
	registerFunction(addOnName, "hide_combat", function()
		RPTAGS.utils.config.set("HIDE_COMBAT", not RPTAGS.utils.config.get("HIDE_COMBAT"))
	end)
	registerFunction(addOnName, "moveframes", function()
		RPTAGS.utils.config.set("LOCK_FRAMES", not RPTAGS.utils.config.get("LOCK_FRAMES"))
	end)
	registerFunction(addOnName, "editor", function()
		RPTAGS.cache.Editor:Show()
	end)
end)

Module:WaitUntil("UTILS_TAGS", function(self, event, ...)
	local RPTAGS = RPTAGS
	...
	local function testTags(s)
		...
		for tag in string.gmatch(s, "%[([^%]]+)%]") do
			if RPTAGS.utils.tags.exists(tag) then
				table.insert(good, tag)
			else
				table.insert(bad, tag)
				err = true
			end --if
		end -- do
		return err, good, bad
	...
	local function fixTagErrors(s)
		...
		for _, badTag in ipairs(bad) do
			s = s:gsub("%[" .. badTag .. "%]", "%[err%]" .. badTag .. "%[/err%]")
		end
		return s
	end -- function
	...
	local function registerTag(tagName, tagMethod, extraEvents, ...)
		...
		if not tagMethod then
			print(tagName, tagMethod)
		end
		if
			not oUF.Tags.Events[tagName] and not tagName:match("%(.+%)$")
	...
	local function registerTagSizeVariants(tagName, tagMethod, tagExtraEvents, ...)
		...
		RPTAGS.utils.tags.registerTag(tagName, function(u1, u2, size)
			return sizeTrim(tagMethod(u1, u2), size)
		end, tagExtraEvents)
	...
	local function colorTagHandler(result, xform, param)
		...
		local function rel(orig, amount) -- orig = 0..1, amount = -1..1
			if amount < 0 then
				return orig * math.abs(1 + amount)
			else
				return orig + amount * (1 - orig)
			end
		end
		...
		local func_hash = {
			V = {
				rel = function(c, n)
					local h, s, v = c:hsv()
					return LibColor.hsv(h, s, rel(v, n))
				end,
				set = function(c, n)
					local h, s, v = c:hsv()
					return LibColor.hsv(h, s, n)
				end,
				add = function(c, n)
					local h, s, v = c:hsv()
					return LibColor.hsv(h, s, v + n)
				end,
			},
			H = {
				rel = function(c, n)
				...
				set = function(c, n)
					local h, s, L = c:hsl()
					return LibColor.hsl(n * 360, s, L)
				end,
				add = function(c, n)
					local h, s, L = c:hsl()
					return LibColor.hsl(h + n * 360, s, L)
				end,
			},
			S = {
				rel = function(c, n)
					local h, s, L = c:hsl()
					return LibColor.hsl(h, rel(s, n), L)
				end,
				set = function(c, n)
					local h, s, L = c:hsl()
					return LibColor.hsl(h, n, L)
				end,
				add = function(c, n)
					local h, s, L = c:hsl()
					return LibColor.hsl(h, s + n, L)
				end,
			},
			L = {
				rel = function(c, n)
					local h, s, L = c:hsl()
					return LibColor.hsl(h, s, rel(L, n))
				end,
				set = function(c, n)
					local h, s, L = c:hsl()
					return LibColor.hsl(h, s, n)
				end,
				add = function(c, n)
					local h, s, L = c:hsl()
					return LibColor.hsl(h, s, L + n)
				end,
			},
		}
		...
		param = (param == "half") and "0.5"
			...
			or (param == "lotsless") and "-50%"
			or param
		if xform:len() * param:len() == 0 then
			return result
		end
    ...
    elseif percent                  then return "|cff" .. select(4, doTransform(result, "rel" .. transform.what, value))
		...
		local newCol
		if transform.er and percent then
			newCol = func_hash[transform.what].rel(col, value)
		elseif transform.er then
			newCol = func_hash[transform.what].add(col, value)
		elseif equal then
			newCol = func_hash[transform.what].set(col, value)
		elseif not (plus or minus) then
			newCol = func_hash[transform.what].set(col, value)
		elseif percent then
			newCol = func_hash[transform.what].rel(col, value)
		else
	...
	local function registerColorTag(tagName, tagMethod, tagExtraEvents, ...)
		...
		RPTAGS.utils.tags.registerTag(tagName, function(u1, u2, transform, p1, p2, ...)
	...
	local function refreshFrame(frameName, ...)
		if RPTAGS.utils.UnitFrames[frameName] then
...
Module:WaitUntil("after CORE_STATE", function(self, event, ...)
	RPTAGS.oUF = _G[C_AddOns.GetAddOnMetadata(addOnName, "X-oUF")] -- let RP_Tags use our oUF for previews
end)
