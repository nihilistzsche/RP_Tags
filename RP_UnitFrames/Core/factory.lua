...
Module:WaitUntil("after MODULE_J", function(self, event, ...)
	local oUF = _G[C_AddOns.GetAddOnMetadata(addOnName, "X-oUF")] -- auto-added by oUF
	local CONST = RPTAGS.CONST
	local frameNames = CONST.RPUF.FRAME_NAMES
	local oUF_style = CONST.RPUF.OUF_STYLE

	if not RPTAGS.utils.config.get("DISABLE_BLIZZARD") then
	...
	oUF:Factory(function(self)
		...
		for unit, frameName in pairs(frameNames) do
			local u = unit:lower()
	...
	EventsFrame:SetScript("OnEvent", function(self, event, isLogin, isReload)
		-- this makes sure everything gets properly applied to our unitframes
		-- i.e., applying fonts once they're loaded
		if isLogin or isReload then
			for frameName, frame in pairs(RPTAGS.cache.UnitFrames) do
				RPTAGS.utils.frames.RPUF_Refresh("all", "everything")
			end
		end
	end)
end)

Module:WaitUntil("MODULE_E", function(self, event, ...)
	local Config = RPTAGS.utils.config
	local Get = Config.get

	local function scale_of(frame)
		return Get(frame.unit:upper() .. "FRAME_SCALE")
	end

	local function scaleFrame(frame)
		if type(frame) == "string" then
			frame = RPTAGS.cache.UnitFrames[frame:lower()]
		end

		if frame then
			frame:SetScale(scale_of(frame))
		end
	end

	local function scaleAllFrames()
		for frameName, frame in pairs(RPTAGS.cache.UnitFrames) do
			frame:SetScale(scale_of(frameName))
		end
	end
...
Module:WaitUntil("before MODULE_F", function(self, event, ...)
	...
	local function RPUF_RegisterLayout(layoutName, layout)
		...
		if layoutCache.known[layoutName] then
			return error("This layout is already registered: " .. layoutName)
		end
		...
		local required_frame_methods = { "GetFrameDimensions" }

		local missing_panel_methods = {}
		local missing_frame_methods = {}

		for _, method in ipairs(required_panel_methods) do
			if not layout.panel_methods[method] and not layout.panel_method_hash[method] then
				table.insert(missing_panel_methods, method)
			end
		end

		for _, method in ipairs(required_frame_methods) do
			if not layout.frame_methods[method] then
				table.insert(missing_frame_methods, method)
			end
		end

		if #missing_panel_methods + #missing_frame_methods == 0 then
			layoutCache.known[layoutName] = layout
			...
			print("Error: layout " .. layoutName .. "  is incomplete")
			for _, method in ipairs(missing_panel_methods) do
				print(" - Missing required panel method " .. method)
			end
			for _, method in ipairs(missing_frame_methods) do
				print(" - Missing required frame method " .. method)
			end
	...
	local function RPUF_RegisterAsDefault(self)
		if self.size then
			RPTAGS.cache.layouts.defaults[self.size] = self.name
		else
			print("Could not register as default")
		end
	end
	...
	local function RPUF_NewLayout(layoutName, layoutSize, layoutVersion)
		return {
			...
			offset = {},
			size = layoutSize,
			version = layoutVersion or C_AddOns.GetAddOnMetadata(addOnName, "Version"),
			GetLayoutSize = function(self)
				return self.size
			end,
			Register_Panel_Method_Hash = function(self, hashName, hashTable)
				self.panel_method_hash[hashName] = hashTable
			end,
			Register_Panel_Method = function(self, funcName, func)
				self.panel_methods[funcName] = func
			end,
			Register_Frame_Method = function(self, funcName, func)
				self.frame_methods[funcName] = func
			end,
			RegisterLayout = function(self)
				RPUF_RegisterLayout(layoutName, self)
			end,
			RegisterAsDefault = function(self)
				RPUF_RegisterAsDefault(self)
			end,
	...
	local function RPUF_Refresh(frameName, ...)
		local function helper(funcName, frame) -- takes a name of a method (from the list above)
			if frame:HasPublicFunction(funcName) then
				frame:Public(funcName)
			end
		end

		for _, param in ipairs({ ... }) do
			local what = refreshFuncs[param:lower()]
			if what then
				local ufCache = RPTAGS.cache.UnitFrames
				if (not frameName) or (frameName:lower() == "all") then
					for name, frame in pairs(ufCache) do
						helper(what, frame)
					end
				elseif ufCache[frameName:lower()] then
					helper(what, ufCache[frameName:lower()])
				end
			end
		end
	end
	...
	local function RPUF_EnableOrDisableBlizzard()
		local blizzFrames =
			{ player = PlayerFrame, target = TargetFrame, focus = FocusFrame, targetTarget = TargetTargetFrame }
		for _, frame in pairs(blizzFrames) do
			if RPTAGS.utils.config.get("DISABLE_BLIZZARD") then
				UnregisterUnitWatch(frame)
			else
	...
	local function RPUF_LayoutHashTable(layoutSize)
		...
		if not layoutSize or layoutSize == "large" then
			for layoutName, _ in pairs(layoutCache.known) do
				hashTable[layoutName:upper()] = loc("RPUF_" .. layoutName:upper())
			end
		elseif layoutCache.by_size[layoutSize] then
			for _, layoutName in ipairs(layoutCache.by_size[layoutSize]) do
				hashTable[layoutName:upper()] = loc("RPUF_" .. layoutName:upper())
			end
		end
		return hashTable
