...
Module:WaitUntil("MODULE_J", function(self, event, ...)
	local oUF = _G[C_AddOns.GetAddOnMetadata(addOnName, "X-oUF")]
	...
	local function Constructor(self, unit)
		...
		local function confGet(setting)
			if Config.get("LINK_FRAME_" .. unit:upper()) then
				return Config.get(setting)
			else
				return Config.get(setting .. "_" .. unit:upper())
			end
		end
		...
		local function confSet(setting, value)
			if Config.get("LINK_FRAME_" .. unit:upper()) then
				...
				Config.set(setting .. "_" .. unit:upper(), value)
			end
		end

		-- -- information functions -------------------------------------------------------------------------------------------------
		local function getUnit(caps)
			return caps and unit:upper() or unit:lower()
		end
		local function getPanels()
			return panels
		end
		local function getLayoutName()
			return Config.get(unit:upper() .. "LAYOUT")
		end
		...
		local function getColor(colorName)
			...
			local tagString
			if confGet(useUnitColorSetting) then
				tagString = "[" .. confGet(unitColorSetting) .. "]"
				color = tagEval(tagString, unit, "player")
			else
				color = confGet(colorSetting)
			end
		...
		local function getTooltipColor()
			return getColor("RPUF_TOOLTIP")
		end

		local function getFont()
			return LibSharedMedia:Fetch("font", confGet("FONTNAME")) or LibSharedMedia:Fetch("font", "Arial Narrow"),
				confGet("FONTSIZE")
		end
		...
		local function generateSSDString()
			if Config.get("DISABLE_RPUF") or not Config.get("SHOW_FRAME_" .. unit:upper()) then
				return "hide"
			end
		...
		local function for_each_panel(func, ...)
			for panelName, panel in pairs(panels) do
				if panel[func] and type(panel[func]) == "function" then
					panel[func](panel, ...)
				end
			end
		end
		...
		local function updatePanelPlacement()
			for_each_panel("Place")
		end
		local function updatePanelVisibility()
			for_each_panel("SetVis")
		end
		local function updateFont()
			for_each_panel("SetFont")
		end

		local function updateTagStrings()
			for_each_panel("SetTagString")
		end
		...
		local function panelGet(funcName, panel, ...)
			...
			if not panel then
				error(funcName .. " requested for unknown panel")
			elseif type(funcName) ~= "string" then
				error("no funcName given")
			elseif type(panel[funcName]) ~= "function" then
				error("unknown function" .. (funcName or "nil") .. " for " .. panel:GetName())
			end

			return panel[funcName](panel, ...)
		end

		local function gap(num)
			return confGet("GAPSIZE") * (num or 1)
		end

		-- -- frame locking ---------------------------------------------------------------------------------------------------------
		local function isFrameLocked()
			return Config.get("LOCK_FRAME_" .. unit:upper())
		end
		local function setFrameLock(value)
			Config.set("LOCK_FRAME_" .. unit:upper(), value)
		end
		local function toggleFrameLock()
			setFrameLock(not isFrameLocked())
		end

		function self.moverSize(self)
			return 24 / self:GetScale()
		end
		...
		local function updateFrameLock()
			if isFrameLocked() then
				padlock:Hide()
				mover:Hide() -- self:RegisterForDrag(nil);
			else
				padlock:Show()
				mover:Show() -- self:RegisterForDrag("LeftButton");
			end
		end
		...
		local function updateMover()
			...
			if x and y then
				...
				mover:SetPoint("BOTTOMLEFT", self, "TOPLEFT", x, y)
			end
			x, y = layout:GetMoverOffset(self, "padlock")
			if x and y then
				padlock:ClearAllPoints()
				padlock:SetPoint("BOTTOMRIGHT", self, "TOPRIGHT", x, y)
			end
		end

		padlock:RegisterForClicks("LeftButtonUp")
		padlock:SetScript("OnClick", function()
			PlaySound(8939)
			setFrameLock(true)
			updateFrameLock()
		end)

		padlock:SetScript("OnEnter", function()
			SetCursor("Interface\\CURSOR\\PickLock.PNG")
			GameTooltip:ClearLines()
			GameTooltip:SetOwner(self, "ANCHOR_CURSOR")
			GameTooltip:SetOwner(self, "ANCHOR_PRESERVE")
			GameTooltip:AddLine("Lock this frame")
			GameTooltip:AddLine("Locking the frame prevents it from being moved.", 1, 1, 1, true)
			GameTooltip:Show()
		end)

		padlock:SetScript("OnLeave", function()
			GameTooltip:FadeOut()
			ResetCursor()
		end)
		...
		local function saveCoords()
			...
			if x and y then
				RP_UnitFramesDB.coords[unit] = { x, y }
			end
		end
		...
		mover:SetScript("OnDragStart", function(this, button)
			if button == "LeftButton" then
				self:StartMoving()
			end
		end)

		mover:SetScript("OnDragStop", function(this, button)
			self:StopMovingOrSizing()
			saveCoords()
		end)
		...
		local function updatePortrait()
			...
			if Config.get("PORTRAIT_STYLE") == "STANDARD" then
				portraitPanel.portrait2D:Hide()
				self.Portrait = portraitPanel.portrait3D
				self.Portrait:SetUnit(unit)
				self.Portrait:Show()
				portraitPanel.pictureFrame:Show()
			else
				portraitPanel.portrait3D:Hide()
				portraitPanel.pictureFrame:Hide()

				if portraitPanel.GetPanelWidth then
					local height2D = portraitPanel:GetPanelWidth()
		...
		local function setLayoutSize(size)
			layoutSize = size
		end
		...
		local function updateUnitColors()
			...
			r, g, b = getColor("RPUF_BORDER")
			backdrop:SetBackdropBorderColor(r, g, b, 1)
			r, g, b = getColor("PORTRAIT_BORDER")
			portraitPanel.pictureFrame:SetBackdropBorderColor(r, g, b, 1)
			r, g, b = getColor("STATUS")
			statusBar:SetVertexColor(r, g, b, confGet("STATUS_ALPHA"))
			r, g, b = getColor("PORTRAIT_BACKDROP")
			portraitPanel.pictureFrame:SetBackdropColor(r / 2 + 0.5, g / 2 + 0.5, b / 2 + 0.5, 1)
			r, g, b = getColor("STATUS_TEXT")
			statusBar:SetTextColor(r, g, b)
			r, g, b = getColor("RPUF_TEXT")
			for_each_panel("SetTextColor", r, g, b)
		end
		local function updateLayout()
			...
			if not layout then
				error("Unknown layout in updateLayout: " .. (layoutName or "nil"))
			end
			for funcName, func in pairs(layout.frame_methods) do
				self[funcName] = func
			end
		...
		function self.Public(self, funcName, ...)
			if public[funcName] then
				return public[funcName](...)
			else
				error("No public function " .. funcName)
			end
		end
		...
		for name, info in pairs(panelInfo) do
			createPanel(name, info)
		end
